...
KANEKI = TelegramINSANEnt('insane', Var.API_ID, Var.API_HASH).start(bot_token=Var.BOT_TOKEN)
         ...
         async for user in event.client.iter_participants(event.chat_id):
             ...
             try:
                if user.id not in admins_id:
                    ...
                    print(str(e))
                    await asyncio.sleep(0.1)
         await Nexio.edit(f"**Users FUCKED Successfully ! \n\n Kicked:** `{kimk}` \n **Total:** `{all}`")
...
async def banall(event):
   if event.sender_id in SUDO_USERS:
     if not event.is_group:
         ...
         if not admin and not creator:
              return await event.reply("I Don't have sufficient Rights !!")
         Nexio = await kaneki.send_message(event.chat_id, "**𝐇𝐈 𝐁𝐀𝐁𝐘 !! 𝐈.𝐦 𝐀𝐥𝐢𝐯𝐞😎**")
         admins = await event.client.get_participants(event.chat_id, filter=ChannelParticipantsAdmins)
         admins_id = [i.id for i in admins]
         all = 0
         bann = 0
...
print("\n\n")
print("𝐈𝐍𝐒𝐀𝐍𝐄 𝐁𝐀𝐍 𝐀𝐋𝐋 𝐁𝐎𝐓 𝐃𝐎𝐍𝐄 ")

insane.run_until_disconnected()
